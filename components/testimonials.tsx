...
export default function Testimonials() {
  ...
  // Функция для получения категорий от сегодняшнего дня до 10 следующими днями
  const getCategoriesForCurrentMonth = () => {
    const categories = [];
    for (let i = 0; i < 10; i++) {
      categories.push(currentDay + i);
    }
    return categories;
  };
  ...
  const handleButtonClick = async (category: number) => {
    ...
    startDate.setDate(startDate.getDate() + category - 1); // Дата начала на основе категории
    const endDate = new Date(startDate);
    endDate.setDate(startDate.getDate() + 6); // Конечная дата через 6 дней от начала

    // Отправляем запрос
    const city = "Санкт-Петербург"; // Город
    const tags = ["йога"]; // Пример тегов
     // const data = await fetchTestimonials(startDate.toISOString().split("T")[0], endDate.toISOString().split("T")[0], city, tags);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6">
      <div className=" py-2 md:py-2">
        <div>
          <div className="mx-auto max-w-3xl pb-0 text-center">
            ...
            <div className="relative inline-flex flex-wrap justify-center rounded-[1.25rem] bg-gray-800/40 p-1">
              {categories.map((category :number) => {
                ...
                return (
                    <button
                        key={category}
                        className={`flex h-8 flex-1 items-center gap-2.5 whitespace-nowrap rounded-full px-3 text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring focus-visible:ring-indigo-200 ${isHighlighted ? "bg-gradient-to-b from-gray-900 via-gray-800/60 to-gray-900" : "opacity-65 transition-opacity hover:opacity-90"}`}
                        aria-pressed={isHighlighted}
                        onClick={() => {
                          if (start === null) {
                            // Если start еще не выбран, устанавливаем его
                            setStart(category);
                            setEnd(null);  // Сбрасываем end, если start выбрано заново
                          } else if (end === null) {
                            // Если end еще не выбран, устанавливаем его
                            setEnd(category);
                          } else {
                            // Если start и end уже выбраны, меняем start
                            setStart(category);
                            setEnd(null); // Сбрасываем end
                          }
                        }}
                    >
                      <span>{category}</span>
          ...
          <div className="flex justify-center pb-12 max-md:hidden md:pb-16">
            <div className="relative inline-flex flex-wrap justify-center rounded-[1.25rem] bg-gray-800/40 p-1">
              ...
              >
                ...
                <input
                    type="text"
                    placeholder="Search..."
                    className="flex-1 px-3 py-1 text-sm rounded-full bg-gray-800 text-white border border-transparent focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </button>
            ...
            <div className="relative mx-2 inline-flex flex-wrap justify-center rounded-[1.25rem] bg-gray-800/40 p-1">
...
const fetchTestimonials = async (startDate: string, endDate: string, city: string, tags: string[]) => {
  try {
